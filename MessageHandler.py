...
class MessageHandler:
    ...
    def on_callback_query(self, msg):
        ...
        if type is 1:
            ...
            self.bot.answerCallbackQuery(query_id, Messages.mark_watched[user.language])
        elif type is 2:
            show = self.db.get_show(data[1])
            self.db.subscribe_user(from_id, data[1])
            self.bot.sendMessage(from_id, Messages.subscribe_success(
            ...
            self.bot.sendMessage(from_id, Messages.unsubscribe_success(
                show.name, user.language
            ), reply_markup=Keyboards.main(user.id, self.admin.is_admin(user.id)))
            self.bot.answerCallbackQuery(query_id, Messages.success[user.language])
            self.user_step[from_id] = 0
    ...
    def on_chat_message(self, msg):
        ...
        logging.info('Got a message for chat: %s %s %s', content_type, chat_type, chat_id)
        user = msg['from']
        id = user['id']
        logging.info('%s', user)
        ...
        try:
            ...
            if command == '/start':
                ...
                self.bot.sendMessage(id, Messages.language_chosen[lan],
                ...
                for episode in us.episodes_to_watch:
                    self.bot.sendMessage(id, '{3}\nseason {1} - episode{2}\n{0}'.format(
                        episode.name, episode.season, episode.number, episode.show.name
                    ), reply_markup=Keyboards.episode(episode.id, lan))
